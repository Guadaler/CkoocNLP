package algorithms.svm;


import algorithms.svm.libsvm.*;
...
public class SVMTool {
	...
	private Problem sProblem;
	// svm参数，里面有svm支持向量机的类型和不同 的svm的核函数类型
	private Parameter sParam;
	 ...
	 * @return
	 */
	private Problem initSvmProblem() {
		List<Double> label = new ArrayList<Double>();
		List<Node[]> nodeSet = new ArrayList<Node[]>();
		getData(nodeSet, label, trainDataPath);

		int dataRange = nodeSet.get(0).length;
		Node[][] datas = new Node[nodeSet.size()][dataRange]; // 训练集的向量表
	 ...
	 * @return
	 */
	private Parameter initSvmParam() {
		// 定义svm_parameter对象
	...
	public void svmPredictData(String testDataPath) {
		...
		List<Node[]> testnodeSet = new ArrayList<Node[]>();
		getData(testnodeSet, testlabel, testDataPath);
		int dataRange = testnodeSet.get(0).length;

		Node[][] testdatas = new Node[testnodeSet.size()][dataRange]; // 训练集的向量表
		...
		// 训练SVM分类模型
		Model model = SVM.svm_train(sProblem, sParam);
	 ...
	 *            数据文件地址
	 */
	private void getData(List<Node[]> nodeSet, List<Double> label,
						 String filename) {
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				Node[] vector = new Node[datas.length - 1];
